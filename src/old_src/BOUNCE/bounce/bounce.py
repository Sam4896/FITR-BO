...
class Bounce:
    ...
    def __init__(
        ...
        """
        optional seed for initial point sampling (sample_init); ensures reproducibility per trial
        """
        self.dtype = None
        """
    ...
    def run(
        ...
        Args:
            iteration_callback: Optional callable(iteration, n_evals, best_min, batch_y, best_before_min)
                called after each iteration (and after initial sample). For logging only; does not
                change algorithm behavior. batch_y is None on initial call.
        ...
        if iteration_callback is not None:
            best_min = self.fx_tr.min().item()
            iteration_callback(iteration, self._n_evals, best_min, None, None)

        while self._n_evals <= self.maximum_number_evaluations:
            best_before_min = self.fx_tr.min().item()
            axus = self.random_embedding
            x = self.x_tr
            fx = self.fx_tr

            # normalize data (original Bounce: recompute mean/std each iteration)
            mean = torch.mean(fx)
            std = torch.std(fx)
            if std == 0:
                std += 1
            ...
            if self.batch_size > 1:
                ...
                sampler = SobolQMCNormalSampler(Size([QMC_SAMPLE_SHAPE]), seed=self._n_evals)
            else:
            ...
            iteration += 1
            if iteration_callback is not None:
                best_after_min = self.fx_tr.min().item()
                batch_y_list = y_next.detach().cpu().reshape(-1).tolist()
                iteration_callback(
                    iteration, self._n_evals, best_after_min, batch_y_list, best_before_min
                )
