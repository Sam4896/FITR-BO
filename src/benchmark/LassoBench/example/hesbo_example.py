...
# Fix SSL for dataset downloads
import sys
import os

sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
from src.benchmark.fix_libsvm_ssl import *
    ...
    if n_jobs > 1:
        ...
        def run_parallel_hesbo(low_dim, high_dim, initial_n, total_itr, test_func, ARD):
            def parallel_hesbo(n_seed):
                _, elapsed0, _, loss0, _, config0 = hesbo_run(
                    low_dim=low_dim,
                    high_dim=high_dim,
                    initial_n=initial_n,
                    total_itr=total_itr,
        ...
        hesbo_objective = run_parallel_hesbo(
            low_dim=eff_dim,
            high_dim=d,
            initial_n=n_doe,
            total_itr=n_total - n_doe,
            test_func=evaluate_hesbo,
            ARD=ARD,
        )
        ...
        if n_repeat > 1:
            ...
            for i in range(n_repeat):
                _, elapsed0, _, loss0, _, _ = hesbo_run(
                    low_dim=eff_dim,
                    high_dim=d,
                    initial_n=n_doe,
                    total_itr=n_total - n_doe,
