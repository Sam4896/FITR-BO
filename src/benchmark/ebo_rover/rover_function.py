...
# added by Fujitsu Limited ####################################################################
class RoverTrajectory(BaseTestProblem):
    def __init__(self, n=30, noise_std=None, negate=False):
        def l2cost(x, point):
        ...
        # maximum value of f
        f_max = 5.0
        self.func = ConstantOffsetFn(domain, f_max)
        self.func = NormalizedInputFn(self.func, raw_x_range)
        x_range = self.func.get_range()
    ...
    def _evaluate_true(self, X):
        ...
        for i in range(batch_size):
            x = X[i].cpu().numpy()
            value = self.func(self.lb + (self.ub - self.lb) * x)
...
def create_large_domain(
    ...
    domain = RoverDomain(
        ...
        s_range=np.array([[-0.1, -0.1], [1.1, 1.1]]),
        rnd_stream=np.random.RandomState(0),
        n_points=n_points,
    )
    return domain
